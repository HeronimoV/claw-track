            ...
            {months.map(([month, data]) => (
              <div key={month} className="flex items-center gap-3">
                ...
                <div className="flex-1 flex gap-0.5 h-6">
                  <div className="bg-[#DC2626] rounded-l h-full transition-all" style={{ width: `${(data.setup / maxMonthly) * 100}%` }} title={`Setup: ${fmt(data.setup)}`} />
                  <div className="bg-[#DC2626]/40 rounded-r h-full transition-all" style={{ width: `${(data.recurring / maxMonthly) * 100}%` }} title={`Recurring: ${fmt(data.recurring)}`} />
                </div>
                <span className="text-xs font-medium text-gray-700 w-16 text-right">{fmt(data.setup + data.recurring)}</span>
              </div>
            ))}
            <div className="flex gap-4 mt-2 text-[10px] text-gray-400">
              <span className="flex items-center gap-1"><span className="w-3 h-3 bg-[#DC2626] rounded" /> Setup</span>
              <span className="flex items-center gap-1"><span className="w-3 h-3 bg-[#DC2626]/40 rounded" /> Recurring</span>
            </div>
          </div>
        )}
      </div>
              ...
              {industries.map(([ind, count]) => (
                <div key={ind} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#DC2626] rounded-full" style={{ width: `${(count / maxIndustry) * 100}%` }} />
                  </div>
            ...
            <div className="space-y-2">
              {sources.map(([src, count]) => (
                <div key={src} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#DC2626]/70 rounded-full" style={{ width: `${(count / maxSource) * 100}%` }} />
                  </div>
                  <span className="text-xs font-medium text-gray-700 w-8 text-right">{count}</span>
                </div>
              ))}
